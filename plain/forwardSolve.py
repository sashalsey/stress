...
class ForwardSolve:
    def __init__(self, outputFolder, outputFolder2, beta, penalisationExponent, variableInitialisation, rho0, pnorm, scenario, initial_stressintegral):
        ...
        self.t_heat = 500.0 # time between deposit and next layer
    ...
    def Setup(self):
        self.nx, self.ny = 1, 10
        self.lx, self.ly =  0.01, 0.1
        # self.nx, self.ny = 10, 20
        # self.lx, self.ly =  0.5, 1.0
        ...
        # self.mech_bc1 = FixedDirichletBC(self.displace, fd.Constant((0.0, 0.0)), np.array([0.25, 0.0]))
        self.mech_bc1 = fd.DirichletBC(self.displace, fd.Constant((0, 0)), 1) 
        self.mech_bc2 = fd.DirichletBC(self.displace, fd.Constant((0, 0)), 2) 
        self.mech_bc3 = fd.DirichletBC(self.displace, fd.Constant((0, 0)), 3) 
        self.mech_bc4 = fd.DirichletBC(self.displace, fd.Constant((0, 0)), 4)
    ...
    def Solve(self, designVariables):
        ...
        if identicalVariables is False:
            ...
            self.T_n.interpolate(fd.Constant(self.T_1))
            self.tempFile.write(self.T_n)
            # Here, new temp is T_n, previous temp is T_n1
            thermal_strain = self.alpha_1 * (self.T_n - self.T_n1) * self.rho_hat * Id
            ...
            fd.solve(a_t == L_t, sol, bcs=[self.mech_bc3, self.mech_bc4])
            ...
            print("u_t max dat {:.4e}".format(np.max(np.abs(sol.dat.data))))
            ...
            while current_time < self.t_heat:
                ...
                fd.solve(F == 0, self.T_n1, bcs=[self.temp_bc3, self.temp_bc4])
            ...
            plt.figure()
            plt.plot(time_steps, max_stress, 'b-o')
            plt.xlabel('Time [s]')
            ...
            # plt.figure(figsize=(10, 6))
            # plt.plot(time_steps, max_stress, 'k-', marker='o', markersize=5, label='Max Mech Stress')
            # plt.xlabel('Time (s)')
            # plt.ylabel('Mech Stress (Pa)')
            # plt.grid(True, linestyle='--', alpha=0.6)
            # plt.tight_layout()
            # plt.show(block=False)
            # plt.pause(10)
