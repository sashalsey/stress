    ...
    def Solve(self, designVariables):
        ...
        if identicalVariables is False:
            ...
            def thermal_strain_solve(self, T_n1, T_n):
                thermal_strain = self.alpha_1 * (T_n1 - T_n) * self.rho_dep * Id
            ...
            def distortion_solve(self):
                ...
                a_t = fd.inner((self.E * strain_total(u_t)), strain_total(v_t))* self.rho_dep * fd.dx # * self.rho_dep diverged!
                ...
                self.total_strain.interpolate(fd.project(strain_total(sol)* self.rho_dep, self.DG0))
                self.total_strainFile.write(self.total_strain) 

                self.strain_mech.interpolate((self.total_strain - self.strain_therm) * self.rho_dep) 
            ...
            def store_stuff(self):
                ...
                max_strain_to.append(flatten_stuff(self.total_strain.dat.data))
                ...
                max_strain_ti.append(flatten_stuff(self.strain_th_increment.dat.data))
                avg_strain_to.append(np.mean(self.total_strain.dat.data))
                avg_strain_me.append(np.mean(self.strain_mech.dat.data))
                avg_strain_th.append(np.mean(self.strain_therm.dat.data))
                avg_strain_ti.append(np.mean(self.strain_th_increment.dat.data))
            ...
            for i in range(n):
                ...
                thermal_strain_solve(self, self.T_n, self.T_n1)
            ...
            plt.plot(time_steps, avg_strain_ti, 'r:', label='Avg Thermal Strain Increment')
            plt.xlabel('Time [s]')
