...
class ForwardSolve:
    ...
    def Solve(self, designVariables):
        ...
        if identicalVariables is False:
            ...
            self.T_n.interpolate(fd.Constant(self.T_0)) # Intial Condition: atmospheric temperature
            ...
            for i in range(n):
                ...
                self.tempFile.write(self.T_n) # Here, new temp is T_n, previous temp is T_n1
                thermal_strain = self.alpha * (self.T_n - self.T_n1) * self.rho_dep * Id
                self.strain_th_increment.interpolate(fd.project(thermal_strain, self.DG0))
                self.strain_th_incrementFile.write(self.strain_th_increment)
                self.strain_therm.interpolate(self.strain_th_increment + self.strain_therm)
                self.strain_thermFile.write(self.strain_therm)
                ...
                # self.strain_mech.interpolate(total_strain - self.strain_therm) 
                ...
                # print("Total strain max dat {:.4e}".format(np.max(np.abs(total_strain.dat.data))))
                ...
                while current_time < self.t_heat:
                    ...
                    # self.strainincrement.interpolate(self.alpha_1 * (self.T_n1 - self.T_n) * self.rho_dep)
                    ...
                    thermal_strain = self.alpha * (self.T_n1 - self.T_n) * self.rho_dep * Id
                    self.strain_th_increment.interpolate(fd.project(thermal_strain, self.DG0))
                    self.strain_th_incrementFile.write(self.strain_th_increment)
                    self.strain_therm.interpolate(self.strain_th_increment + self.strain_therm)
                    self.strain_thermFile.write(self.strain_therm)
                ...
                if i == n - 1: # Forced cooled to ambient
                    self.T_n1.interpolate(self.T_0) # Here, new temp is T_n1, previous temp is T_n
                    thermal_strain = self.alpha * (self.T_n1 - self.T_n) * self.rho_dep * Id
                    self.strain_th_increment.interpolate(fd.project(thermal_strain, self.DG0))
                    self.strain_th_incrementFile.write(self.strain_th_increment)
                    self.strain_therm.interpolate(self.strain_th_increment + self.strain_therm)
