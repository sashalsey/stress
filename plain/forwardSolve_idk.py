...
class ForwardSolve:
    ...
    def Setup(self):
        ...
        # boundary conditions
        # 1: vertical y lhs, 2: vertical y rhs, 3: horizontal x bottom, 4: horizontal x top# Do collection of points
        ...
        self.strain_th_incrementFile = VTKFile(self.outputFolder + "0strain_th_increment.pvd")

        self.strain_therm = fd.Function(self.DG0, name="strain_therm")
        self.strain_thermFile = VTKFile(self.outputFolder + "1strain_therm.pvd")

        self.total_strain = fd.Function(self.DG0, name="total_strain")
        self.total_strainFile = VTKFile(self.outputFolder + "2strain_total.pvd")

        self.strain_mech = fd.Function(self.DG0, name="strain_mech")
        self.strain_mechFile = VTKFile(self.outputFolder + "3strain_mech.pvd")
    ...
    def Solve(self, designVariables):
        ...
        if identicalVariables is False:
            ...
            def distortion_solve(T_n1, T_n, E, rho, alpha):
                ...
                fd.solve(a_t == L_t, sol, bcs=[self.mech_bc1, self.mech_bc2, self.mech_bc3])
            ...
            for i in range(n):
                ...
                a_t = fd.inner((self.E * strain_total(u_t)), strain_total(v_t)) * self.rho_dep * fd.dx
                L_t = fd.inner((self.E * self.strain_therm), strain_total(v_t)) * self.rho_dep * fd.dx
