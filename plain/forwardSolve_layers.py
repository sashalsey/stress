...
class ForwardSolve:
    ...
    def Setup(self):
        ...
        self.total_strain = fd.Function(self.DG0, name="total_strain")
    ...
    def Solve(self, designVariables):
        ...
        if identicalVariables is False:
            ...
            def distortion_solve(T_n1, T_n, E, rho, alpha):
                thermal_strain = alpha * (T_n1 - T_n) * rho * Id
            ...
            for i in range(n):
                ...
                thermal_strain = self.alpha_1 * (self.T_n - self.T_n1) * self.rho_dep * Id
                self.strain_th_increment.interpolate(fd.project(thermal_strain, self.DG0))
                self.strain_therm.interpolate(self.strain_th_increment + self.strain_therm)
                self.strain_thermFile.write(self.strain_therm)

                self.E = self.E_0 + (self.E_1 - self.E_0) * (self.rho_dep ** self.penalisationExponent) 
                a_t = fd.inner((self.E * strain_total(u_t)), strain_total(v_t)) * fd.dx
                L_t = fd.inner((self.E * self.strain_therm), strain_total(v_t)) * fd.dx

                sol = fd.Function(self.displace)
                fd.solve(a_t == L_t, sol)
                ...
                while current_time < self.t_heat:
                    ...
                    thermal_strain = self.alpha_1 * (self.T_n1 - self.T_n) * self.rho_dep * Id
                    self.strain_th_increment.interpolate(fd.project(thermal_strain, self.DG0))
                    self.strain_therm.interpolate(self.strain_th_increment + self.strain_therm)
                    self.strain_thermFile.write(self.strain_therm)
